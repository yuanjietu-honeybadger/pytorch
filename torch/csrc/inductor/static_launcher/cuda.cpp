...
inline CUdeviceptr getPointerFast(PyObject* obj) {
  ...
  // Fast path: direct C++ tensor access — no Python attribute lookup,
  // no PyObject_Call, no cuPointerGetAttribute.
  return reinterpret_cast<CUdeviceptr>(THPVariable_Unpack(obj).data_ptr());
}
...
static PyObject*
FastCudaLauncher_new(PyTypeObject* type, PyObject* args, PyObject* kwds) {
  HANDLE_TH_ERRORS
  auto* self =
  ...
  TORCH_CHECK(
      nTotal <= MAX_ARGS,
      "_FastCudaLauncher: too many arguments (",
      nTotal,
      " > ",
      MAX_ARGS,
      ")");
...
static PyObject* fast_launcher_vectorcall(
  ...
  // Pack kernel args from Python into argStorage.
  for (int i = 0; i < self->numKernelArgs; ++i) {
    ...
    switch (typeChar) {
      ...
      case 'd':
        ...
        break;
      default:
        TORCH_CHECK(false, "_FastCudaLauncher: unknown arg type '", typeChar, "'");
    }
  }
