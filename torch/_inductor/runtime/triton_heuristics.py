...
def check_autotune_cache(
    ...
    local_enabled = not force_disabled or bool(
    ...
    if (
        ...
        autotune_cache = AutotuneCache.create(
            inductor_meta, filename, configs_hash,
            fn_src=fn_src,
            local_enabled=local_enabled, remote_enabled=remote_enabled,
        )
        if autotune_cache:
...
class CachingAutotuner(KernelInterface):
    ...
    def recheck_autotune_cache(
        ...
        if (
            len(self.compile_results) == 1
            and self.compile_results[0].runtime_winner
        ):
            return
    ...
    def precompile(
        ...
        with self.lock:
            ...
            is_runtime_winner = (
                len(self.compile_results) == 1
                and getattr(self.compile_results[0], "runtime_winner", False)
            )
