...
def _convert_element_type(x: TensorBox, dtype: torch.dtype):
    ...
    src_dtype = x.get_dtype()
    low_pr_fp = (torch.bfloat16, torch.float16)
    # Preserve explicit casts to/from fp16 and bf16 even when precision-cast
    # emulation is disabled. This keeps round-trips like x.to(bf16).float()
    # from being silently elided during fusion. See #179561.
